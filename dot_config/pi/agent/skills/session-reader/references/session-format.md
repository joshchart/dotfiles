...
Sessions are stored in `~/.config/pi/agent/sessions/` organized by project:
...
```json
{
  "dir": "~/.config/pi/agent/sessions/<project>/subagent-artifacts",
  "files": [
    {
...
3. **Persistent artifacts** — `details.artifacts.files[]` in `~/.config/pi/agent/sessions/<project>/subagent-artifacts/` (persistent)
