...
Parse pi session JSONL files into readable, structured output. Sessions live in `~/.config/pi/agent/sessions/<project>/` as `.jsonl` files.

## Step 1: Identify the Session File

Resolve the session file path. Sessions are stored at:
```
~/.config/pi/agent/sessions/--<path-with-dashes>--/<timestamp>_<uuid>.jsonl
```

If the user provides a partial path or project name, find the file:
```bash
ls -t ~/.config/pi/agent/sessions/*<project>*/*.jsonl | head -5
```
...
uv run ${CLAUDE_SKILL_ROOT}/scripts/read_session.py ~/.config/pi/agent/sessions/<project>/subagent-artifacts/<hash>_worker.jsonl --mode overview
